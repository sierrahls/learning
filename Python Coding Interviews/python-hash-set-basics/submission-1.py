...
def check_keys(hash_set: Set[str], keys: List[str]) -> List[bool]:
    empty = []
    for key in keys: #look through keys
        if key in hash_set: #we check if the key is in the hash set
            empty.append(True)
        else:
            empty.append(False)

    return empty
    #we dont need to unpack 2 variables because that just creates 2 tuples
